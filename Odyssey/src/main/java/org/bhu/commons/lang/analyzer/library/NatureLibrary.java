...
public class NatureLibrary {
	...
	private static final HashMap<String, Double> NATURERELATION = new HashMap<String, Double>();
	/**
	...
	* @throws
	...
	private static void init() throws IOException {
		...
		reader = StaticDictionaryLoad.getNatureTableReader();
		int j = 0;
		while ((temp = reader.readLine()) != null) {
		...
		reader = StaticDictionaryLoad.getNatureRelationReader();
		while ((temp = reader.readLine()) != null) {
			if (StringUtil.isBlank(temp))
				continue;
			strs = temp.split(split);
